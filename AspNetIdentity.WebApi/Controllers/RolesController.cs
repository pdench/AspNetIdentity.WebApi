...
namespace AspNetIdentity.WebApi.Controllers
{
    ...
    public class RolesController : BaseApiController
    {
        ...
        public async Task<IHttpActionResult> Create(CreateRoleBindingModel model)
        {
            ...
            Uri locattionHeader = new Uri(Url.Link("GetRoleById", new { id = role.Id }));
            return Created(locattionHeader, TheModelFactory.Create(role));
        ...
        public async Task<IHttpActionResult> ManageUsersInRole(UsersInRoleModel model)
        {
            ...
            {
                var appUser = this.AppUserManager.FindByIdAsync(user);
                ...
                if (!result.Succeeded)
                {
                    ModelState.AddModelError("", String.Format("UYser {0} could not be removed from role", user));
                }
            }
            if (!ModelState.IsValid)
            {
